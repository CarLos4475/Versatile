...
- **L10n**: `AppLocalizations` via gen-l10n. **Source files are the `.arb` files** (`app_en.arb`, `app_es.arb`). The `.dart` files are auto-regenerated on build. Always add strings to the `.arb` files first, then update the `.dart` files manually or run `flutter gen-l10n`.
